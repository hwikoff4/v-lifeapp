...
export default function WorkoutSession() {
  ...
  const [exercises, setExercises] = useState<Exercise[]>([
    { id: 1, name: "Bench Press", sets: 4, reps: "8-10", rest: "90", completed: false, completedSets: 0 },
    { id: 2, name: "Pull-Ups", sets: 3, reps: "8-10", rest: "90", completed: false, completedSets: 0 },
    { id: 3, name: "Shoulder Press", sets: 3, reps: "10-12", rest: "60", completed: false, completedSets: 0 },
    { id: 4, name: "Bicep Curls", sets: 3, reps: "12", rest: "60", completed: false, completedSets: 0 },
    { id: 5, name: "Tricep Extensions", sets: 3, reps: "12", rest: "60", completed: false, completedSets: 0 },
  ])
  ...
  const completeSet = () => {
    if (workoutMode === "sets") {
      if (currentSet < currentExercise.sets) {
        ...
        const updatedExercises = exercises.map((ex) => (ex.id === currentExercise.id ? { ...ex, completed: true } : ex))
        setExercises(updatedExercises)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-black to-charcoal pb-20">
            ...
            <Card className="mb-6 border-white/10 bg-black/50 backdrop-blur-sm">
              <CardContent className="p-6">
                ...
                <div className="mb-6 aspect-video rounded-lg bg-black/70 flex items-center justify-center">
                  <img
                    src={`/.jpg?key=6b5t3&height=200&width=300&query=${currentExercise.name} exercise demonstration`}
      ...
      <Dialog open={isSwapDialogOpen} onOpenChange={setIsSwapDialogOpen}>
        <DialogContent className="border-white/10 bg-charcoal text-white">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold">Swap Exercise</DialogTitle>
            <DialogDescription className="text-white/70">
              Choose an alternative exercise for {currentExercise?.name}
            </DialogDescription>
          </DialogHeader>
            ...
            {currentAlternatives.length > 0 ? (
              currentAlternatives.map((alt, index) => (
                <button
                  ...
                  className="w-full rounded-lg border border-white/10 bg-black/30 p-4 text-left transition-all hover:border-accent hover:bg-accent/10"
                >
                  <div className="font-semibold text-white">{alt.name}</div>
                  <div className="text-sm text-white/60">{alt.reason}</div>
                </button>
              ))
            ) : (
              <div className="rounded-lg border border-white/10 bg-black/30 p-4 text-center text-white/60">
      ...
      <Dialog open={isEditRestDialogOpen} onOpenChange={setIsEditRestDialogOpen}>
        <DialogContent className="border-white/10 bg-charcoal text-white">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold">Edit Rest Time</DialogTitle>
            <DialogDescription className="text-white/70">
              Adjust rest time for {currentExercise?.name}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <Label htmlFor="rest-input" className="text-white/80">
                Rest Time (seconds)
              </Label>
              <Input
                id="rest-input"
            ...
            <div className="flex gap-3">
              <ButtonGlow variant="outline-glow" onClick={() => setIsEditRestDialogOpen(false)} className="flex-1">
                Cancel
              </ButtonGlow>
              <ButtonGlow variant="accent-glow" onClick={saveRestTime} className="flex-1">
                Save
              </ButtonGlow>
            </div>
          </div>
